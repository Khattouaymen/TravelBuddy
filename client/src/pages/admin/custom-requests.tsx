...
// Type pour les demandes personnalisées
...
const AdminCustomRequests = () => {
  ...
  const sortedAndFilteredRequests = useMemo(() => {
    ...
    // D'abord, trier par date (plus récent en premier)
    let result = [...requests].sort((a, b) => {
      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    });
    
    // Ensuite, filtrer par statut si un filtre est sélectionné
    if (statusFilter) {
  ...
  const handleUpdateStatus = async (id: number, status: string) => {
    try {
      ...
      if (status === "completed") {
        ...
        // Mettre à jour l'objet sélectionné pour éviter de fermer la modal
  ...
  const formatDateSafely = (dateString: string, formatStr: string = "d MMMM yyyy") => {
    try {
      // Essayer de parser la date avec parseISO pour une meilleure compatibilité
              ...
              <Tabs defaultValue="info" className="w-full">
                ...
                <ScrollArea className="max-h-[calc(90vh-200px)]">
                  ...
                  <TabsContent value="details" className="space-y-4 p-1">
                    ...
                    {/* Notes internes - Nouvelle fonctionnalité */}
                    <Card>
                      <CardHeader className="pb-2">
